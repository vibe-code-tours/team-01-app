...
export default function SchedulePage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Load user profile for address/phone defaults
      const profile = await userFetch<{ phone: string | null; address: string | null }>("/profile");
      if (profile.success && profile.data) {
        if (profile.data.phone) setContactPhone(profile.data.phone);
        if (profile.data.address) setDeliveryAddress(profile.data.address);
      }
  ...
  async function handleBook() {
    ...
    const result = await userFetch(`/orders/${orderId}/schedule`, {
      method: "POST",
  ...
  if (!orderId) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-base-content/50">Please go to an order to schedule delivery.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      <h1 className="text-2xl font-bold mb-2">Schedule Delivery</h1>
      <p className="text-base-content/50 mb-8">Pick a date and time slot for your delivery.</p>
